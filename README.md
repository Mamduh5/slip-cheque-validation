...
- Before comparing reference and date/time values in the duplicate assessment, OCR comparison normalization is applied: reference normalization resolves common O/0, I/1, l/1 character confusions in the digit portions of Thai bank reference codes; Thai date/time normalization collapses tesseract spacing fragmentation around Thai month abbreviations so `6 พ . ค . 69 17:52` and `6 พ.ค. 69 17:52` compare as equal. Raw stored values are not modified. The normalizations prevent false suppression (OCR noise treated as a conflict) and false matching (spacing variants of the same date treated as different).
